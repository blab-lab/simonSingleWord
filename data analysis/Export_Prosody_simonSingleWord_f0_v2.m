...
Syllable = repelem("1", height(f0))'; %Syllable
Speaker = repelem(Participant, height(f0))'; %Speaker
Duration = repelem(dataVals(nTrial).dur{1, Indx1}, height(f0))';
Phase = repelem(expt.listConds(nTrial), height(f0))'; %Phase

% Create tables with headers
data = table(f0, Trial, Position, Word, Syllable, Speaker, Duration, Phase);
...
while ParticipantIndex <= nParticipants
    ...
    while nTrial <= nRow
        ...
        Position = (1:height(f0))'; %Position within a measurement window
        Word = repelem(expt.listWords(nTrial), height(f0))'; %Word
        Syllable = repelem("A", height(f0))'; %Syllable
        Speaker = repelem(Participant, height(f0))'; %Speaker
        Duration = repelem(dataVals(nTrial).dur{1, Indx1}, height(f0))';
        Phase = repelem(expt.listConds(nTrial), height(f0))'; %Phase

        % Create a temporary table with headers
        temp = table(f0, Trial, Position, Word, Syllable, Speaker, Duration, Phase);
        ...
        f0 = dataVals(nTrial).f0{1, Indx2}; %Int
        Trial = repelem(nTrial, height(f0))'; %Trial number
        Position = (1:height(f0))'; %Position within a measurement window
        Word = repelem(expt.listWords(nTrial), height(f0))'; %Word
        Syllable = repelem("B", height(f0))'; %Syllable
        Speaker = repelem(Participant, height(f0))'; %Speaker
        Duration = repelem(dataVals(nTrial).dur{1, Indx2}, height(f0))';
        Phase = repelem(expt.listConds(nTrial), height(f0))'; %Phase
